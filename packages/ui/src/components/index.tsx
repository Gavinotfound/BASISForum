...
export const Navbar = ({ user, onSignIn, onSignOut, onSearch, onProfile, onBookmarks, onNotifications, onHome, onBulletin, onCreatorDesk, onStudy, onKnowledge }: NavbarProps) => {
  const { t } = useLanguage();
  return (
      ...
      <Container maxWidth={false} sx={{ maxWidth: { lg: 1440, xl: 1680 }, px: { xs: 2, sm: 3, md: 4, lg: 5 } }}>
        <Toolbar disableGutters sx={{ minHeight: { xs: 'auto', sm: 60, md: 68 }, py: { xs: 0.75, sm: 0 }, display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'auto minmax(0, 1fr)' }, gap: { xs: 0.75, sm: 2 }, alignItems: 'center' }}>
          ...
          <Box aria-label="Forum navigation" sx={{ minWidth: 0, width: { xs: '100%', sm: 'auto' }, display: 'flex', justifyContent: { xs: 'flex-start', sm: 'flex-end' }, gap: { xs: 0.35, sm: 0.75 }, alignItems: 'center', overflowX: 'auto', overscrollBehaviorX: 'contain', pb: { xs: 0.25, sm: 0 }, '&::-webkit-scrollbar': { display: 'none' } }}>
            ...
            <Button sx={{ color: 'text.primary', flex: '0 0 auto', px: { xs: 0.8, md: 1 }, minWidth: 0, minHeight: 44 }} onClick={onStudy}>STUDY</Button>
            <Button sx={{ color: 'text.primary', flex: '0 0 auto', px: { xs: 0.8, md: 1 }, minWidth: 0, minHeight: 44 }} onClick={onKnowledge}>KNOWLEDGE</Button>
            {user ? <>
              ...
              <Button sx={{ color: 'text.primary', flex: '0 0 auto', px: { xs: 0.8, md: 1 }, minWidth: 0, minHeight: 44 }} onClick={onCreatorDesk}>CREATE</Button>
