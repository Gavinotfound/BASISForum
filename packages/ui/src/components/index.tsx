...
export const RegisterForm = ({ action }: { action: (previousState: RegisterFormState, formData: FormData) => Promise<RegisterFormState> }) => {
  const { t } = useLanguage();
  const [state, formAction, isPending] = React.useActionState(action, {});
  return <Card sx={{ maxWidth: 400, mx: 'auto', mt: 8, p: 2 }}><CardContent><Typography variant="h5" sx={{ mb: 3, fontWeight: 800, textAlign: 'center' }}>{t('auth.join')}</Typography><Box component="form" action={formAction} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}><Typography variant="body2" color="text.secondary">{t('auth.fullName')}</Typography><input name="name" type="text" required style={{ padding: '12px', borderRadius: 0 }} /><Typography variant="body2" color="text.secondary">{t('auth.email')}</Typography><input name="email" type="email" required style={{ padding: '12px', borderRadius: 0 }} /><Typography variant="body2" color="text.secondary">{t('auth.password')}</Typography><input name="password" type="password" required style={{ padding: '12px', borderRadius: 0 }} />{state.error ? <Typography role="alert" variant="body2" color="error">{state.error}</Typography> : null}<Button type="submit" variant="contained" color="primary" size="large" sx={{ mt: 2 }} disabled={isPending}>{isPending ? t('auth.creating') : t('auth.create')}</Button></Box></CardContent></Card>;
};
