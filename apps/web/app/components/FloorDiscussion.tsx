...
export default function FloorDiscussion({ starterId, starterAuthor, comments, canReply, action, voteAction, canVote, reportAction, acceptedReplyId, canResolve, acceptAction, canBlock, currentUserId, blockAction, blockedUserIds }: FloorDiscussionProps) {
  ...
  const renderComment = (comment: FloorComment, floor: number, depth: 0 | 1) => {
    ...
    return <Box key={comment.id} id={`comment-${comment.id}`} sx={{ ml: { xs: depth ? 1 : 0, sm: depth ? 1.5 : 0, md: depth ? 3 : 0 }, mt: depth ? { xs: 1, md: 1.5 } : 0, position: 'relative', isolation: depth ? 'isolate' : undefined, zIndex: depth ? 0 : undefined, '&:before': depth ? { content: '""', position: 'absolute', left: { xs: -8, sm: -11 }, top: 0, bottom: 0, width: 1, bgcolor: 'var(--bf-divider)' } : {} }}>
      <Box component="article" sx={{ p: { xs: 1.5, sm: 2.25, md: 2.75 }, position: 'relative', zIndex: 1, bgcolor: depth ? 'var(--bf-surface)' : 'transparent', color: 'var(--bf-text) !important', borderTop: '1px solid var(--bf-divider)', borderLeft: depth ? '1px solid var(--bf-divider)' : 'none', transition: 'background-color 120ms linear', '& > :not(style)': depth ? { position: 'relative', zIndex: 1 } : undefined, '&:hover': { bgcolor: depth ? 'var(--bf-surface)' : 'var(--bf-hover)' }, ...(depth ? { '& .MuiBox-root': { position: 'relative', zIndex: '5 !important' }, '& .MuiTypography-root, & .MuiButton-root': { position: 'relative', zIndex: '10 !important', opacity: '1 !important', color: 'var(--bf-text) !important' }, '& .MuiTypography-caption, & .MuiTypography-overline': { color: 'var(--bf-muted) !important' } } : {}) }}>
