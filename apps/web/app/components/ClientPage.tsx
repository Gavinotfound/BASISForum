...
export default function ClientPage({ user, threads, sort, campaign, featuredBulletin }: { user?: ForumUser; threads: ThreadListItem[]; sort: 'latest' | 'hot'; campaign: ForumCampaignSlot; featuredBulletin?: FeaturedBulletinPost | null }) {
  ...
  return <Box sx={{ maxWidth: { lg: 1440, xl: 1680 }, mx: 'auto' }}>
    ...
    <Box component="nav" aria-label="Forum sections" sx={{ mt: { xs: 2, md: 2.5 }, display: 'flex', alignItems: 'center', gap: 0, borderTop: '1px solid var(--bf-divider)', borderBottom: '1px solid var(--bf-divider)', overflowX: 'auto', '&::-webkit-scrollbar': { display: 'none' } }}><Button size="small" aria-current="page" sx={{ flex: '0 0 auto', minHeight: 44, color: 'var(--bf-selection-fg)', bgcolor: 'var(--bf-text)', '&:hover': { color: 'var(--bf-selection-fg)', bgcolor: 'var(--bf-text)' } }} onClick={() => router.push('/')}>DISCUSSIONS</Button><Button size="small" sx={{ flex: '0 0 auto', minHeight: 44, color: 'var(--bf-text)' }} onClick={() => router.push('/knowledge')}>KNOWLEDGE BASE</Button></Box>
